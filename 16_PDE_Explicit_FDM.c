...
int main() {
    ...
    for (i = 0; i <= nt; i++) u[i] = (float*)calloc(nx+1, sizeof(float));
    for (i = 0; i <= nx; i++) u[0][i] = sin(M_PI * i * dx / L); // IC: sin(πx/L)
    for (j = 0; j < nt; j++) {
        u[j][0] = u[j][nx] = 0;                                  // BC: u=0 at ends
        for (i = 1; i < nx; i++)
    ...
    for (i = 0; i <= nt; i++) free(u[i]); free(u);
}
